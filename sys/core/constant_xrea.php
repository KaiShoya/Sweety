<?php
define("PUBLIC_TOP", '/sweety');
define("ASSETS_PATH", PUBLIC_TOP . '/assets');

define("TOP_PATH", __DIR__ . '/../..');
define("PUB_PATH", TOP_PATH . '/public_html');
define("SYS_PATH", TOP_PATH . '/sys');

// タイトル
define("INDEX", 'トップ');
define("HOTEL_LIST", 'ホテル一覧');
define("PRICE_LIST", '価格一覧');
define("TOS", '利用規約');
define("AVAILABILITY", "空室状況");
define("PRICE_CHANGE", "料金変更");
define("LOGIN", "ログイン");
define("LOGOUT", "ログアウト");

// DB情報
define("DB_TYPE", 'mysql'); // mysql or pgsql.
define("DB_HOST", 'localhost');
define("DB_NAME", 'sweetyhotel');
define("DB_USER", 'sweetyhotel');
define("DB_PASS", 'awe09fjwr');
