...
var app = new Vue({
  ...
  methods: {
    ...
    get_prices: function(e) {
      ...
      //Ajaxリクエスト
      axios
        .post('api/price_list.php', formData)
        .then(function(res) {
          res.data.forEach((value, i) => {
            app.prices.push({
              id: value.id,
    ...
    change_is_available: function(e) {
      if (this.isAvailable.length > 0) {
        params = app.create_params('available', this.isAvailable)
      } else {
        params = app.create_params('available', undefined)
      }
      window.location.href = window.location.pathname + '?' + params
    }
  },
